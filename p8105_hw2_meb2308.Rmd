...
```
There are `r count(distinct(subway_df, station_name))` distinct stations, and 73 are ADA compliant. Among the `r count(filter(subway_df, vending == "NO"))` stations without vending machines, `r count(filter(subway_df, vending == "NO", entry == "YES"))` allow entry.
